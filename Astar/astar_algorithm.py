...
class robot:
	def __init__(self, position, orientation, resolution, blocked):
	...
	def move(self, goal):
		dist2robot = [self.resolution, math.sqrt(2)*self.resolution]
		...
		check = position((0,0))
		for i in range(8):
			check.x = self.pose.position.x + x_sgn[i] * 10
			check.y = self.pose.position.y + y_sgn[i] * 10
			check2 = check.toTuple()
			if check2 in self.visited:
				penalizacion = self.visited.count(check2)*self.resolution*5
			else:
				penalizacion = 0
			if check2 not in self.blocked:
